...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xep="http://www.renderx.com/XEP/xep">
  ...
  <xsl:template name="do-page">
    ...
    <page>
      ...
      <!-- Added "x" prefix because get-value uses "start-with" rather than exact comparison. -->
      <xsl:attribute name="chapter-ctr">
        <xsl:call-template name="get-value">
          <xsl:with-param name="key" select="'xchapter-ctr'"/>
        </xsl:call-template>
      </xsl:attribute>
      ...
      <xsl:attribute name="revised">
        ...
        <xsl:choose>
          <!-- RS: The new revdate (from the Revised date on the first page) is empty in our test sample, so this condition always fails.
          ...
          <xsl:when test=".//xep:text[@value='__revst__' or @value='__revend__'] and normalize-space($revdate)=normalize-space($new-revdate)">
           -->
          <!--  -->
			<!-- RS: Special new marker ("__revcont__") to handle cases where an ancestor output the change bar, but there's no __revst__ or __revend__ marker -->
      ...
      <xsl:variable name="label">
        <xsl:value-of select="//xep:internal-bookmark[@destination-id = $target-id]/@label"/>
      </xsl:variable>
      <xsl:attribute name="label" select="$label"/>
      -->
      <!-- EM version from ATA FO: -->
      <xsl:variable name="label">
      	<xsl:choose>
      	  <!-- 5-level version ("Page Block" prefix): -->
      	  <xsl:when test="//xep:internal-bookmark[@destination-id = $target-id]/@label[starts-with(.,'Page Block')]">
	        <xsl:value-of select="substring-after(//xep:internal-bookmark[@destination-id = $target-id]/@label[starts-with(.,'Page Block')],' - ')"/>
      	  </xsl:when>
      	  <!-- 3-level version (ATA number prefix): -->
      	  <xsl:otherwise>
      	    <!-- Find the label starting with an ATA number, and extract from after the ATA number (from the 9th character) -->
	        <xsl:value-of select="substring(//xep:internal-bookmark[@destination-id = $target-id]/@label[matches(.,'\d\d-\d\d-\d\d .*')],9)"/>
      	  </xsl:otherwise>
      	</xsl:choose>
      </xsl:variable>
      <xsl:if test="$label != ''">
