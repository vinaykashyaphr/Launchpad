			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:if test="not($bp or $cp)
						...
						and not((following::*)[1]/self::pmEntry[count(ancestor::pmEntry) &lt; 3])
						...
						<xsl:if test="$debug-figure">
							<xsl:message>Breaking after figure (id: <xsl:value-of select="@id"/>). First following: <xsl:value-of select="name((following::*)[1])"/></xsl:message>
						</xsl:if>
					</xsl:if>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template name="standard-sheet">
			...
			<fo:block text-align="center" padding="0pt" keep-together.within-page="always">
				<xsl:choose>
					...
					<xsl:when test="not($ap) and (ancestor::pmEntry/@pmEntryType='pmt58' or  $bp or $cp)">
					</xsl:when>
					<xsl:otherwise>
				...
				<!-- UPDATE: If an ancestor proceduralStep was changed, and the figure is are the last thing in -->
				<!-- the procedural step, we want to end the change bar for the procedural step after the last graphic's -->
				<!-- title, since a page break is usually added after the figure, which would cause the change bar -->
				<!-- incorrectly to end on the following page. This was detected by seeing an extra asterisk in the LEP. -->
				...
				<!-- NOTE: If you change this condition, you need to also update the one at the end of proceduralSteps -->
				<!-- in S1000DLists.xsl (in the proceduralStep template). -->
				<xsl:if test="not(parent::figure/legend) and self::graphic[last()]
					and (not(ancestor::proceduralStep/@changeMark='0') and 
					  (ancestor::proceduralStep/@changeType='add' or ancestor::proceduralStep/@changeType='modify'))">
					
					<xsl:message>Last graphic in a changed proceduralStep; checking if we need to end the change bar</xsl:message>
					
					<!-- Test to see if the proceduralStep's last element is a figure, and it is the same as this graphic's figure. -->
					<xsl:variable name="lastElement" select="(ancestor::proceduralStep/descendant::*)[last()]"/>
					
					<xsl:if test="$lastElement/ancestor-or-self::figure
						and count($lastElement/ancestor-or-self::figure intersect parent::figure) = 1">
						<xsl:message>The last element in the proceduralStep is this figure, so end the change bar</xsl:message>
						<xsl:call-template name="cbEnd" />
					</xsl:if>
				</xsl:if>
