...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- S1000DHelper is our Saxon extension; used here for IPD figure variant numbering (skipping "I" and "O"). -->

  <xsl:attribute-set name="header.detail">
    <xsl:attribute name="padding-before">3pt</xsl:attribute><!-- (top) --><!-- 4 -->
    <xsl:attribute name="padding-after">2pt</xsl:attribute>
    <xsl:attribute name="padding-start">0pt</xsl:attribute><!-- (left) --><!-- 4 -->
    <xsl:attribute name="padding-end">0pt</xsl:attribute><!-- 4 -->
  ...
  <xsl:attribute-set name="standard.cell">
    <xsl:attribute name="padding-before">0pt</xsl:attribute><!-- (top) -->
    <xsl:attribute name="padding-after">0pt</xsl:attribute><!-- -1pt -->
  ...
  <!-- Main template to generate the Numerical Index -->
  <xsl:template name="build-numerical-index">
     ...
     <xsl:if test="$niExtractSorted/numindex/part">
  ...
  <xsl:template name="do-index">
    ...
    <!-- Start a new page sequence for the Numerical Index -->    
    ...
    <fo:table border-style="none" text-align="left">
      ...
      <!-- Original column settings from CMM version for reference; for EIPC we add the CH-SECT-UNIT column. -->
      ...
      <fo:table-column column-number="1" column-width="3in"/>
      <fo:table-column column-number="2" column-width="1.25in"/><!-- Airline Stock No (not used) -->
      <fo:table-column column-number="3" column-width="1in"/>
      <fo:table-column column-number="4" column-width="0.375in"/>
      <fo:table-column column-number="5" column-width="0.6in"/>
      <fo:table-column column-number="6" column-width="0.5in"/>
      
      <fo:table-header font-size="10pt"><!-- CMM had 10pt; but original EIPC XSL uses 8pt. Styler also uses 10pt. -->
        <fo:table-row>
        ...
        <fo:table-row>
          ...
          <fo:table-cell xsl:use-attribute-sets="header.detail" padding-before="0pt">
             <fo:block text-align="left"><!--  margin-left="-0.15in" -->CH-SECT-UNIT</fo:block>
          </fo:table-cell>
          <fo:table-cell xsl:use-attribute-sets="header.detail" number-columns-spanned="2" text-align="center">
            <fo:block margin-left="-0.1in"><!--  margin-left="-0.2in" -->FIG.</fo:block>
            <fo:block margin-left="-0.1in"><!--  margin-left="-0.2in" -->ITEM</fo:block>
          </fo:table-cell>
          <fo:table-cell xsl:use-attribute-sets="header.detail">
  ...
  <xsl:template name="do-rest-of-row">
    <xsl:choose>
      ...
      <fo:table-cell xsl:use-attribute-sets="standard.cell">
         <xsl:choose>
            <xsl:when test="@pn = preceding-sibling::part[1]/@pn 
                         and @csu-key = preceding-sibling::part[1]/@csu-key">
               <fo:block>&#160;</fo:block>
            </xsl:when>
            <xsl:otherwise>
               <fo:block text-align="left">
                  <xsl:value-of select="@csu-key"/>
               </fo:block>
            </xsl:otherwise>
         </xsl:choose>
      </fo:table-cell>
    ...
    <xsl:document>
      <numindex>
        ...
        <xsl:for-each select="/pm/content/pmEntry[@pmEntryType='pmt75']//itemSeqNumber/partRef[not(contains(@partNumberValue,'ORDERNHA')) and not(contains(@partNumberValue,'NONPROC'))]">
          ...
          <!-- UPDATE: Actually Styler uses a calculated attribute (with property set "Set figureNumber"), so we should also calculate it here -->
          <!-- rather than relying on the attributes being correct. -->
          ...
          <!-- Same logic as in eipcMiscFunctions (calc-figure-number-param) -->
          <xsl:variable name="figureNumber" select="count(ancestor::pmEntry[1]/preceding-sibling::pmEntry)+1"/>
          ...
          <xsl:variable name="figureNumberVariant">
          ...
          <!--  For EIPC add the CHAP-SECT-UNIT number -->
          <xsl:variable name="chapNo" select="ancestor::catalogSeqNumber[1]/@systemCode"/>
          <xsl:variable name="sectNo" select="concat(ancestor::catalogSeqNumber[1]/@subSystemCode, ancestor::catalogSeqNumber[1]/@subSubSystemCode)"/>
          <xsl:variable name="unitNo" select="ancestor::catalogSeqNumber[1]/@assyCode"/>
          <xsl:variable name="ataNo" select="concat($chapNo, '-', $sectNo, '-', $unitNo)"/>
            ...
            <!-- <xsl:sort select="@partNumberValue"/> -->
              ...
              <xsl:if test="parent::itemSeqNumber/genericPartDataGroup/genericPartData[@genericPartDataName='opt']">
	              ...
	              <part>
	                  ...
	                  <xsl:attribute name="fig" select="$figureNumber"/><!-- number(ancestor::catalogSeqNumber[1]/@figureNumber) -->
	                  <xsl:attribute name="figureNumberVariant" select="$figureNumberVariant"/>
		              ...
		              <!-- The original EIPC FO used "csu-key" for the ATA number -->
		              <xsl:attribute name="csu-key" select="$ataNo"/>
	                  <!-- <xsl:attribute name="isalpha">
	                    <xsl:variable name="firstChar" select="substring(genericPartDataValue,1,1)"/>
	                    <xsl:choose>
	                      <xsl:when test="string(number($firstChar)) = 'NaN'">
