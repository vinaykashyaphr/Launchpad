...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:rx="http://www.renderx.com/XSL/Extensions">
  ...
  <xsl:template name="intro-toc">
    ...
    <fo:page-sequence master-reference="Body" font-family="Arial" font-size="10pt" initial-page-number="1">
      ...
      <xsl:call-template name="init-static-content">
        ...
        <xsl:with-param name="suppressAtacode" select="0"/><!-- RS: EM changes to 0, but Styler doesn't show the code for the Intro ToC in EIPC. --><!-- DJH TEST 20090831 -->
              ...
              <fo:table-cell>
                <fo:table border-width="1pt" padding="6pt">
                  ...
                  <fo:table-body>
                    ...
                    <!-- Get the Introduction pmEntry in context (should only be one). -->
                  ...
                  <fo:table-body>
                    ...
                    <xsl:for-each select="pmEntry">
                      ...
                      <xsl:for-each select="pmEntry">
                        ...
                        <!-- Figures can also be nested in another level of pmEntry for numbering purposes: -->
                        <xsl:for-each select="dmContent/dmodule/content/illustratedPartsCatalog/figure | pmEntry/dmContent/dmodule/content/illustratedPartsCatalog/figure">
                          <xsl:call-template name="figure-row"/>
                        </xsl:for-each>
                      </xsl:for-each>
                    </xsl:for-each>
    ...
    <fo:table-row>
      ...
      <fo:table-cell xsl:use-attribute-sets="toc.table.cell" padding-left="{$indent}">
        <fo:block text-align-last="justify">
          <fo:basic-link>
            <!-- The Chapter doesn't create its own page, so link to the first Section page -->
            <xsl:attribute name="internal-destination" select="pmEntry[1]/@id"/><!-- @id -->
      ...
      <fo:table-cell xsl:use-attribute-sets="toc.table.cell" text-align="right">
        <fo:block margin-left="-0.475in">
          <fo:basic-link color="blue">
            <xsl:attribute name="internal-destination" select="pmEntry[1]/@id"/>
            <fo:page-number-citation>
              <xsl:attribute name="ref-id">
                <xsl:value-of select="pmEntry[1]/@id"/>
              </xsl:attribute>
            </fo:page-number-citation>
          </fo:basic-link>
        </fo:block>
  ...
  <xsl:template name="unit-row">
    <xsl:variable name="title" select="concat('Unit ', @authorityDocument, ' - ', upper-case(pmEntryTitle))"/>
    <xsl:call-template name="write-toc-row">
      <xsl:with-param name="title" select="$title"/>
      <xsl:with-param name="indent" select="'2pc'"/>
      <xsl:with-param name="refid" select="@id"/>
  ...
  <xsl:template name="write-toc-row">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$pgblkRow='1'">
        <fo:table-row keep-with-next.within-page="always">
          <fo:table-cell xsl:use-attribute-sets="toc.table.cell" padding-left="{$indent}">
            ...
            <fo:block font-weight="bold" text-align-last="justify" text-indent="-0.25in" start-indent="0.25in">
              <fo:basic-link>
                <xsl:attribute name="internal-destination" select="$refid"/>
                <xsl:value-of select="$title"/>
                <xsl:text>&#160;</xsl:text>
      ...
      <xsl:otherwise>
        <fo:table-row>
          <fo:table-cell xsl:use-attribute-sets="toc.table.cell" padding-left="{$indent}">
            <fo:list-block text-indent="-0.25in" start-indent="0.25in">
              <fo:list-item>
                <fo:list-item-label end-indent="label-end()">
                  <fo:block>
                    <fo:basic-link>
                ...
                <fo:list-item-body start-indent="body-start()">
                  <fo:block text-align-last="justify"><!-- Hanging indent was causing the Intro ToC level numbers to overlap the title: [text-indent="-0.25in" start-indent="0.25in"] -->
                    <fo:basic-link>
                      <xsl:attribute name="internal-destination" select="$refid"/>
                      <xsl:value-of select="$title"/>
                      <xsl:text>&#160;</xsl:text>
