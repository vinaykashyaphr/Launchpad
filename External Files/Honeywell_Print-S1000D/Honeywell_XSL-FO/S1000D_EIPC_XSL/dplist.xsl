      ...
      <fo:table border="none" font-size="10pt" margin-bottom=".125in">
        ...
        <fo:table-body>
          ...
          <xsl:for-each select="catalogSeqNumber/itemSeqNumber">
            ...
            <xsl:with-param name="keep_eipc">0</xsl:with-param>
				  </xsl:call-template>
				</xsl:if>
              </fo:table-cell>
  		...
  		<xsl:when test="@refType='rft01' or @refType='rft02' or @refType='rft06' or @refType='rft10' or @refType='rft51' or @refType='rft52' or @refType='rft53' or @refType='rft54'">
  			<xsl:choose>
  				...
  				<xsl:otherwise>
  					...
  					<xsl:value-of select="catalogSeqNumberRef[1]/@systemCode"/>
  					<xsl:text>-</xsl:text> 
  					<xsl:value-of select="catalogSeqNumberRef[1]/@subSystemCode"/>
  					<xsl:value-of select="catalogSeqNumberRef[1]/@subSubSystemCode"/>
  					<xsl:text>-</xsl:text> 
  					<xsl:value-of select="catalogSeqNumberRef[1]/@assyCode"/>
	  				<xsl:text> DPL FIG </xsl:text>
	  				<xsl:call-template name="IPD_ReferToFigNums"/>
	  				<xsl:text> </xsl:text>
	  				<xsl:call-template name="IPD_ReferToTypeText"/>
  					<xsl:text>)</xsl:text> 
	...
	<!-- <fo:block margin-left="{$iplNoteMargin}"> -->
  	...
  	<xsl:choose>
  		...
  		<xsl:when test="$numFigs = 2">
  			...
  			<xsl:text> AND DPL FIG </xsl:text>
  			<xsl:call-template name="getFigNum">
  				<xsl:with-param name="index">2</xsl:with-param>
  			</xsl:call-template>
  		</xsl:when>
  		<xsl:otherwise>
        <xsl:variable name="replace_dpl" select="helper:getFigureRefsEIPC(catalogSeqNumberRef)" />
        <xsl:value-of select='replace($replace_dpl, "null", "DPL")'/>
      </xsl:otherwise>
    </xsl:choose>
