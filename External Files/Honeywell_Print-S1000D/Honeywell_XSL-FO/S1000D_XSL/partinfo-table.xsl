...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:rx="http://www.renderx.com/XSL/Extensions">
  ...
  <xsl:template name="part-info-table">
    <fo:block font-size="20pt" font-weight="bold" margin-left="0.5in" margin-right="0.5in" margin-top="0in"
      ...
      <!-- UPDATE: Test for specific dmCode (from Styler) -->
      <!-- UPDATE: Don't output if there is a "variant title" specified. -->
      <!-- <xsl:if test="/pm/content/pmEntry/@pmEntryType='pmt75'"> -->
      <xsl:if test="/pm/content//dmodule/identAndStatusSection/dmAddress/dmIdent/dmCode[@infoCode='941' and @infoCodeVariant='A']
      	and not($documentType='ipc') and not (/pm/identAndStatusSection/pmAddress/pmAddressItems/externalPubCode[@pubCodingScheme='variantTitle'])">
        ...
        <fo:table-body>
          ...
          <xsl:for-each select="/pm/crossRefTable/productCrossRefTable/product">
		    <fo:table-row>
				...
				<xsl:variable name="numUseForPartsList" select="count(/pm/crossRefTable/applicCrossRefTable/productAttributeList/productAttribute[@useForPartsList='yes'])"/>
				...
				<xsl:choose>
					...
					<xsl:when test="$numUseForPartsList &gt; 0">
					  <xsl:for-each select="/pm/crossRefTable/applicCrossRefTable/productAttributeList/productAttribute[@useForPartsList='yes']">
						...
						<fo:table-cell padding-top="4pt" padding-right="12pt"><!-- padding-right="45pt" -->
						  <!-- If there is only one (column), make it centred. -->
						  <xsl:if test="$numUseForPartsList = 1">
						  	<xsl:attribute name="text-align">center</xsl:attribute>
						  </xsl:if>
						  <fo:block hyphenate="true" hyphenation-push-character-count="4" hyphenation-remain-character-count="4"><!--ID: <xsl:value-of select="@id"/>:-->
					...
					<xsl:otherwise>
					  ...
					  <xsl:variable name="numAttributes" select="count(/pm/crossRefTable/applicCrossRefTable/productAttributeList/productAttribute)"/>
					  <xsl:for-each select="/pm/crossRefTable/applicCrossRefTable/productAttributeList/productAttribute">
						<xsl:variable name="productId" select="@id"/>
						<fo:table-cell padding-top="4pt" padding-right="12pt">
						  <xsl:if test="$numAttributes = 1">
						  	<xsl:attribute name="text-align">center</xsl:attribute>
						  </xsl:if>
						  <fo:block hyphenate="true" hyphenation-push-character-count="4" hyphenation-remain-character-count="4"><xsl:value-of select="$product/assign[@applicPropertyIdent=current()/@id]/@applicPropertyValue"/></fo:block>
						</fo:table-cell>
					  </xsl:for-each>
					</xsl:otherwise>
				</xsl:choose>
  ...
  <xsl:template name="part-table-headers">
	...
	<!-- If the useForPartsList attribute is used on the productAttributes use only those ones. Otherwise, -->
	<!-- assume that all of the productAttribute elements will be used for the part list table. -->
    <xsl:choose>
		<xsl:when test="$numUseForPartsList &gt; 0">
		  <xsl:for-each select="/pm/crossRefTable/applicCrossRefTable/productAttributeList/productAttribute[@useForPartsList='yes']">
  ...
  <!-- Not currently used... leave for reference
