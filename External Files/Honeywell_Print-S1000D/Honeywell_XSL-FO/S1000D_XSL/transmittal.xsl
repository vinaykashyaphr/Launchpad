...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:rx="http://www.renderx.com/XSL/Extensions"
  ...
  <!-- In ATA some of the TRANSMITTAL INFORMATION section was generated here as well, but in S1000D it's authored -->
  <!-- as a regular front-matter section. -->
  

  <xsl:template name="THE-Text-Marker">
	<xsl:variable name="startat" select="ancestor-or-self::pmEntry[last()]/@startat"/>
	<!-- Calculate the pageblock number from the startat attribute. For those in the thousands, use the first -->
	<!-- one or two digits (like "3" for 3000 or "11" for 11000). Otherwise use "0" to indicate no page block. -->
	<xsl:variable name="pageBlock">
		<xsl:choose>
			<xsl:when test="$startat='' or $startat='1' or not(string-length($startat) = 4 or string-length($startat)=5)">0</xsl:when>
			<xsl:when test="string-length($startat) = 4"><xsl:value-of select="substring($startat,1,1)"/></xsl:when>
			<xsl:when test="string-length($startat) = 5"><xsl:value-of select="substring($startat,1,2)"/></xsl:when>
			<!-- The above conditions should catch everything, but just in case, default to "0" (no page block) -->
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
     <!-- This ~THE~ marker ("Table of Highlights Entry") is used by Omnimark (update_toc.xom) to adjust page numbers for point pages -->
	 <!-- and repagination due to foldout tables. -->
	 <!-- UPDATE: Add the pageblock number to make sure we're not replacing page numbers of the wrong page block -->
      <fo:inline font-size="0pt"><xsl:text>~THE</xsl:text><xsl:value-of select="$pageBlock"/><xsl:text>~</xsl:text></fo:inline>
  </xsl:template>
  
  <xsl:template name="pageNumber">
	<xsl:param name="id"/>
		<xsl:text>(Page </xsl:text>
		<xsl:call-template name="THE-Text-Marker"/>
		...
		<!-- Add non-breaking spaces after the closing paren so that the extent of the hot link in the PDF is longer -->
		<!-- in case the page number needs to be updated with a longer point page (so that the link still works at -->
		<!-- the end of the longer page number). -->
		<xsl:text>)&#xA0;&#xA0;&#xA0;&#xA0;&#xA0;&#xA0;</xsl:text>
  </xsl:template>
  ...
  <xsl:template name="figure-change-title">
    ...
    <fo:basic-link>
      ...
      <!-- But IPL Figures are different, outputting like "Detailed Parts List [newline] IPL Figure 1" -->
      <xsl:choose>
      	<xsl:when test="ancestor::illustratedPartsCatalog">
      		<xsl:text>Detailed Parts List</xsl:text>
      		<fo:block><xsl:text>IPL Figure </xsl:text>
		    ...
		    <xsl:call-template name="task-or-subtask"/>
      		...
      		<fo:block>
				<xsl:call-template name="pageNumber">
					<xsl:with-param name="id" select="$page-no-refid"/>
				</xsl:call-template>
  ...
  <xsl:template name="change-title-from-element">
    <!-- <xsl:message>change-title-from-element called with element: <xsl:value-of select="name(.)"/></xsl:message> -->
  	<xsl:choose>
			...
			<xsl:choose>
				...
				<xsl:when test="parent::table">
			        <fo:basic-link>
			          ...
			          <fo:block>
						<xsl:call-template name="pageNumber">
							<xsl:with-param name="id" select="parent::table/@id"/>
						</xsl:call-template>
						<!--
			            <xsl:text>(Page </xsl:text>
		        		<xsl:call-template name="THE-Text-Marker"/>
	    ...
	    <xsl:when test="self::pmEntry">
	        ...
	        <fo:basic-link>
	    	  ...
	    	  <xsl:choose>
	    		...
	    		<!-- Top-level pmEntries use the pmEntryTitle in the title column (no page(?))-->
	    		...
	    		<xsl:otherwise>
	    		  <xsl:choose>
			     	...
			     	<!-- For now when no task is specified in the authorityDocument, just use the top-level pmEntryTitle (uppercase) -->
			     	<xsl:when test="ancestor::pmEntry[last()]/pmEntryTitle">
			     		<fo:block text-transform="uppercase"><xsl:value-of select="ancestor::pmEntry[last()]/pmEntryTitle"/></fo:block>
			     	</xsl:when>
			      </xsl:choose>
	    ...
	    <xsl:when test="self::definitionListItem | self::listItem">
	    	<xsl:choose>
  ...
  <!-- Context: require/assume we're in reasonForUpdate context -->
  <xsl:template name="determine-change-title">
  	
  	<!-- <xsl:message>determine-change-title called for element <xsl:value-of select="name(.)"/>. Parent: <xsl:value-of select="name(..)"/>; ../..: <xsl:value-of select="name(../..)"/>; ../../..: <xsl:value-of select="name(../../..)"/></xsl:message> -->
...
[           <xsl:attribute name="internal-destination" select="parent::PGBLK/@KEY"/> ]
 ...
 -->
	  ...
	  <xsl:when test="parent::pmStatus/parent::identAndStatusSection/parent::pm">
	 	...
	 	<xsl:for-each select="( (/pm/content/pmEntry | /pm/content/pmEntry/pmEntry)[@reasonForUpdateRefIds=$reasonForChangeid] )[1]">
	  ...
	  <!-- UPDATE: When it is an IPL dmodule, treat the same as a change to the IPL figure. -->
	  <xsl:when test="not(ancestor::dmodule//*[@reasonForUpdateRefIds=$reasonForChangeid])">
	  	<xsl:choose>
	  		<xsl:when test="ancestor::dmodule[1]/content/illustratedPartsCatalog/figure">
		  	  <!-- Call change-title-from-element with the IPL figure in context -->
		 	  <xsl:for-each select="ancestor::dmodule[1]/content/illustratedPartsCatalog/figure[1]">
		 	  	<xsl:call-template name="change-title-from-element"/>
		 	  </xsl:for-each>
	  		</xsl:when>
	  		<xsl:otherwise>
  ...
  <xsl:template name="output-paragraph-number">
    ...
    <xsl:variable name="subtaskNumber">
      <xsl:choose>
      	<xsl:when test="ancestor-or-self::levelledPara">
      	  <xsl:for-each select="ancestor-or-self::levelledPara[last()]">
      	  	<xsl:call-template name="calc-list-position"/>
      	  </xsl:for-each>
      	</xsl:when>
      	<xsl:when test="ancestor-or-self::proceduralStep">
      	  <xsl:for-each select="ancestor-or-self::proceduralStep[last()]">
    ...
    <xsl:choose>
      <xsl:when test="count(ancestor-or-self::proceduralStep) &gt; 1">
      	<xsl:text>Step </xsl:text>
      	<xsl:for-each select="ancestor-or-self::proceduralStep[position() &lt; last()]">
      ...
      <xsl:when test="count(ancestor-or-self::levelledPara) &gt; 1">
      	<xsl:text>Step </xsl:text>
      	<xsl:for-each select="ancestor-or-self::levelledPara[position() &lt; last()]">
      		<!-- Same numbering as in S1000DLists.xsl -->
		    <xsl:variable name="listPosition">
		        <xsl:call-template name="calc-list-position"/>
		    </xsl:variable>
  ...
  <xsl:template name="paragraph-numbering-from-element">
    ...
    <xsl:choose>
      ...
      <!-- Same for title, table, and listItem (may need to add other title contexts later) -->
      <xsl:when test="self::title or self::table or self::listItem">
      	<!-- nothing -->
      </xsl:when>
      ...
      <xsl:when test="self::para or self::proceduralStep or self::notePara or self::note">
        <xsl:call-template name="output-paragraph-number"/>
      </xsl:when>
    ...
    <fo:table rx:table-omit-initial-header="true" width="100%">
      ...
      <fo:table-header>
        ...
        <fo:table-row border-after-style="solid" border-after-width="1pt" border-before-style="solid" border-before-width="1pt" display-align="after" font-weight="bold" padding="3pt"
          ...
          <fo:table-cell padding-left="0.125in" padding-right="0.125in" padding-top="0.125in" padding-bottom="0.125in">
            <fo:block>
      ...
      <fo:table-body border-bottom-style="solid" border-bottom-width="1pt">
        ...
        <fo:table-row border-after-style="solid" border-after-width="1pt" border-before-style="solid" border-before-width="1pt" display-align="after" font-weight="bold" padding="3pt"
          ...
          <fo:table-cell padding-top="0.125in" padding-bottom="0.125in" padding-left="0.125in" padding-right="0.125in">
            <fo:block>
              <xsl:text>Description of Change</xsl:text>
            </fo:block>
          </fo:table-cell>
        ...
        <xsl:for-each select="//reasonForUpdate">
          ...
          <xsl:sort select="
            ...
            then
              ...
              then
                count( ((/pm/content/pmEntry | /pm/content/pmEntry/pmEntry)[@reasonForUpdateRefIds=current()/@id])[1]/preceding::*[@reasonForUpdateRefIds] )
              else
          ...
          <xsl:choose>
		    ...
		    <!-- UPDATE: Now reasonForUpdate does not need to be linked, so use all of them. -->
          	<!-- <xsl:when test="ancestor::pmEntry[last()]/following-sibling::pmEntry[@pmEntryType='pmt58']
          		or ancestor::pmEntry[last()]/following-sibling::pmEntry[@pmEntryType='pmt91']
          		or ancestor::dmodule//*[@reasonForUpdateRefIds=$reasonForChangeid]"> -->
          	<!-- <xsl:when test="ancestor::pmEntry[last()]/@isFrontmatter='1'
          		or ancestor::dmodule//*[@reasonForUpdateRefIds=$reasonForChangeid]"> -->
          	<xsl:when test="true()">
  ...
  <xsl:template match="TRANSLTR">
    <fo:page-sequence master-reference="Body" font-family="Arial" font-size="10pt">
      ...
      <fo:flow flow-name="xsl-region-body">
        ...
        <fo:block id="transltr">
          <fo:block text-align="center" font-weight="bold" font-size="12pt" space-after="12pt">
            <xsl:text>TRANSMITTAL INFORMATION</xsl:text>
          </fo:block>
          <fo:block text-align="center" text-decoration="underline" space-after="12pt" text-transform="uppercase">
            <xsl:text>REVISION NUMBER </xsl:text>
            <xsl:value-of select="/CMM/@TSN"/>
    ...
    <xsl:choose>
      <xsl:when test="$documentType = 'acmm'">
        <xsl:message>No RR in ACMMs.</xsl:message>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="rrTable"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="TRANSLTR/PARA[1]">
    <xsl:call-template name="check-rev-start"/>
    <fo:block keep-together.within-page="4" text-transform="uppercase">
      <xsl:apply-templates/>
    </fo:block>
    <xsl:call-template name="check-rev-end"/>
  </xsl:template>
  
  <xsl:template match="TRANSLTR/TABLE[1]">
    <xsl:apply-templates/>
  ...
  <xsl:template match="TRANSLTR/TITLE"/>
   -->
   
  <xsl:template name="irm-highlights-page">
    <!-- Old ATA version for reference:
    <xsl:choose>
      <xsl:when test="/CMM/@CHAPNBR = '72' and /CMM/@SECTNBR = '00'">
        <xsl:choose>
          <xsl:when test="ancestor-or-self::PGBLK[@PGBLKNBR='5000'][number(@CONFNBR) >= 1000]/preceding-sibling::PGBLK[1][@PGBLKNBR='5000']">
            <xsl:text>Inspection/Check, PN&#160;</xsl:text>
	  ...
	  <xsl:choose>
		<xsl:when test="ancestor-or-self::pmEntry[@pmEntryType='pmt65']"><!-- Alternative Repair code (probably just older documents) -->
	       ...
	       <xsl:if test="ancestor-or-self::pmEntry[last()]/@authorityName">
	       	<xsl:text>, PN&#160;</xsl:text><xsl:value-of select="ancestor-or-self::pmEntry[last()]/@authorityName"/>
	       </xsl:if>
		</xsl:when>
		...
		<xsl:otherwise>
			<xsl:text>[[[Unhandled IRM pmEntryType: </xsl:text><xsl:value-of select="ancestor-or-self::pmEntry[last()]/@pmEntryType"/><xsl:text>]]]</xsl:text>
			<xsl:message>[[[Unhandled ToH IRM pmEntryType: <xsl:value-of select="ancestor-or-self::pmEntry[last()]/@pmEntryType"/>]]]</xsl:message>
		</xsl:otherwise>
	  </xsl:choose>    
    <xsl:choose>
    	<xsl:when test="ancestor-or-self::pmEntry[@pmEntryType='']"></xsl:when>
    </xsl:choose>
  </xsl:template>
