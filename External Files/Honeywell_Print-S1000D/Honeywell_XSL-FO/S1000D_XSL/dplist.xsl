      ...
      <fo:table border="none" font-size="10pt" margin-bottom=".125in">
              ...
              <fo:block margin-right="-0.5in">
                ...
                <xsl:value-of select="count(ancestor::pmEntry[1]/preceding-sibling::pmEntry) + 1"/>
		      	...
		      	<xsl:if test="$variantNum &gt; 0">
	      			...
	      			<!-- <xsl:if test="true() or $variantNum &gt; 24">
	      				<xsl:message>TEST: variant number for <xsl:value-of select="$variantNum"/>: <xsl:value-of select="helper:getVariantCode($variantNum)"/></xsl:message>
	      			</xsl:if> -->
        ...
        <fo:table-body>
          <xsl:for-each select="catalogSeqNumber/itemSeqNumber">
			...
			<!-- In some cases there are really long nomenclatures that need to break. Try to detect those cases here and then -->
			<!-- don't set the keep-together attributes. -->
            <xsl:variable name="keepTogether">
            	<xsl:choose>
            		<xsl:when test="count(partLocationSegment/referTo/functionalItemRef) &gt; 150">false</xsl:when>
            ...
            <fo:table-row>
              ...
              <fo:table-cell text-align="left">
		        ...
		        <xsl:if test="not(@changeMark='0') and (@changeType='add' or @changeType='modify' or (@changeType='delete' and @changeMark='1'))
		        	or not(parent::catalogSeqNumber/@changeMark='0') and 
		        	  (parent::catalogSeqNumber/@changeType='add' or parent::catalogSeqNumber/@changeType='modify'
              ...
              <fo:table-cell border="none" text-align="left" padding-left="0pt" padding-top="0pt"
                ...
                <xsl:if test="not(partLocationSegment/notIllustrated)">
                  <fo:block>&#160;<fo:inline margin-left="10pt">
                      ...
                      <xsl:value-of select="ancestor::catalogSeqNumber/@trimmedItem"/>
