...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:rx="http://www.renderx.com/XSL/Extensions"
  ...
  <xsl:template name="highlights_table">
    ...
    <fo:table rx:table-omit-initial-header="true" width="100%">
      <!-- Column widths copied from Styler -->
      <fo:table-column column-number="1" column-width="2.5in"/><!-- 1.75in -->
      <fo:table-column column-number="2" column-width="proportional-column-width(1)"/><!-- 4.25in -->
      <fo:table-column column-number="3" column-width="0.85in"/><!-- .75in -->
      <fo:table-header>
  ...
  <!-- For EM, we list the ATA number, the section title, then the Subtask. -->
  <!-- Called with the changed element in context. -->
  <xsl:template name="task-or-subtask">
     <xsl:choose>
     	<xsl:when test="not($isNewPmc)">
			<!-- For 3-level EM, the ATA number is at the second pmEntry, with an exception for the introduction -->
		    <xsl:if test="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=1]/@authorityDocument">
		     	<fo:block><xsl:value-of select="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=1]/@authorityDocument"/></fo:block>
		    </xsl:if>
     		<xsl:choose>
     			...
     			<xsl:otherwise>
			      <xsl:if test="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=1]/pmEntryTitle">
			     	<fo:block text-transform="uppercase"><xsl:value-of select="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=1]/pmEntryTitle"/></fo:block>
			      </xsl:if>
			     <!-- NOTE: No task/subtask for 3-level(?) -->
     			</xsl:otherwise>
     		</xsl:choose>
     	</xsl:when>
     	<xsl:otherwise>
		     <!-- For 5-level, the ATA number is at the third pmEntry. -->
		     <xsl:if test="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=2]/@authorityDocument">
		     	<fo:block><xsl:value-of select="ancestor::pmEntry[count(ancestor::pmEntry)=2]/@authorityDocument"/></fo:block>
		     </xsl:if>
		     <xsl:if test="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=3]/pmEntryTitle">
		     	<fo:block text-transform="uppercase"><xsl:value-of select="ancestor-or-self::pmEntry[count(ancestor::pmEntry)=3]/pmEntryTitle"/></fo:block>
		     </xsl:if>
		     <xsl:choose>
		     	<xsl:when test="ancestor-or-self::dmContent[1]/preceding-sibling::dmRef[1]/@authorityDocument">
		     		<fo:block>
		     		  <xsl:text>Subtask </xsl:text>
		     		  <xsl:value-of select="ancestor-or-self::dmContent[1]/preceding-sibling::dmRef[1]/@authorityDocument"/>
		     		</fo:block>
		     	</xsl:when>
		     	<!-- For EM, not sure if the TASK ever is used
		     	...
		     	<xsl:otherwise>
		     	  <xsl:message>[ERROR] No task or subtask found</xsl:message>
		     	  <fo:inline color="red">[ERROR: No task or subtask found]</fo:inline>
		     	</xsl:otherwise>
		     </xsl:choose>
     	</xsl:otherwise>
     </xsl:choose>
  	...
  	<xsl:choose>
  		...
  		<xsl:when test="self::table">
	        <fo:basic-link>
	          ...
	          <!-- EM doesn't list the page number
	          <fo:block>
	            <xsl:text>(Page </xsl:text>
	            <xsl:call-template name="page-number-prefix"/>
	            <fo:page-number-citation>
	    ...
	    [UPDATE: May be necessary for EM? Keep for reference.]
...
[           <xsl:attribute name="internal-destination" select="parent::PGBLK/@KEY"/> ]
 ...
 -->
  ...
  <xsl:template name="output-paragraph-number">
    ...
    <!-- UPDATE: It looks like the first paragraph number (numbered "1.") can occur at different -->
    <!-- pmEntry levels, so just look back to the current pmEntry level. -->
    <!-- UPDATE: If it's a three-level PMC, and the last pmEntry is 2nd-level, then it should -->
    <!-- use the proceduralStep numbering instead. -->
    <!-- 
    ...
    <xsl:variable name="taskNumber">
      <xsl:choose>
        <!-- 3-level -->
      	<xsl:when test="not($isNewPmc) and count(ancestor::pmEntry) = 2">
	      <xsl:choose>
    ...
    <xsl:variable name="subtaskNumber">
      <xsl:choose>
        ...
        <!-- 3-level, in two level nesting -->
      	<xsl:when test="not($isNewPmc) and count(ancestor::pmEntry) = 2">
	      <xsl:choose>
	      	<xsl:when test="ancestor-or-self::levelledPara">
	      	  <xsl:for-each select="ancestor-or-self::levelledPara[last()-1]">
	      	  	<xsl:call-template name="calc-list-position"/>
	      	  </xsl:for-each>
	      	</xsl:when>
	      	<xsl:when test="ancestor-or-self::proceduralStep">
	      	  <xsl:for-each select="ancestor-or-self::proceduralStep[last()-1]">
	      	  	<xsl:call-template name="calc-prclist-position"/>
	      	  </xsl:for-each>
	      	</xsl:when>
	      </xsl:choose>
        ...
        <!-- 3-level, in three level nesting -->
      	<xsl:when test="not($isNewPmc)">
	      <xsl:choose>
	      	<xsl:when test="ancestor-or-self::levelledPara">
	      	  <xsl:for-each select="ancestor-or-self::levelledPara[last()]">
    ...
    <!-- UPDATE: For 3-level PMC, need to check if we are in 2 or 3 level pmEntry nesting -->
    <xsl:choose>
    	<xsl:when test="not($isNewPmc) and count(ancestor::pmEntry)=2">
		    <xsl:choose>
		      <xsl:when test="count(ancestor-or-self::proceduralStep) &gt; 2">
		      	<xsl:text>Step </xsl:text>
		      	<xsl:for-each select="ancestor-or-self::proceduralStep[position() &lt; (last()-1)]">
		      ...
		      <xsl:when test="count(ancestor-or-self::levelledPara) &gt; 2">
		      	<xsl:text>Step </xsl:text>
		      	<xsl:for-each select="ancestor-or-self::levelledPara[position() &lt; (last()-1)]">
		      		<!-- Same numbering as in S1000DLists.xsl -->
				    <xsl:variable name="listPosition">
				        <xsl:call-template name="calc-list-position"/>
				    </xsl:variable>
