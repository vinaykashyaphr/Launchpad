...
is styled into the intermediate file -->
  ...
  <!-- The front_body_count is used in ACMM for the total number of pages (not just the front). -->
  <!-- It doesn't seem to have any other usages, so it's a bit of a misnomer (left from the ATA FO). -->
  <xsl:variable name="front_body_count" >
    <xsl:choose>
      <!-- This won't work on the first formatting pass... should set to 0 if there is no LEP extract file. -->
      <xsl:when test="number($LEP_PASS)=1">0</xsl:when>
      <xsl:when test="$documentType = 'acmm'">
  ...
  <!-- <xsl:variable name="g-doc-full-name" select="'COMPONENT MAINTENANCE MANUAL'"/> -->
	<xsl:variable name="g-doc-full-name">
		<xsl:choose>
			...
			<xsl:when test="/pm/@type='cmm'">Component Maintenance Manual</xsl:when>
			<xsl:when test="/pm/@type='fmm'">Flight Line Maintenance Manual</xsl:when>
			<xsl:when test="/pm/@type='irm'">Inspection/Repair Manual</xsl:when>
			<xsl:when test="/pm/@type='orim'">Overhaul And Repair Instruction Manual</xsl:when>
			<xsl:when test="/pm/@type='im'">Installation Manual</xsl:when>
			<xsl:when test="/pm/@type='sdim'">System Description, Installation, and Maintenance Manual</xsl:when>
			<xsl:when test="/pm/@type='sdom'">System Description and Operation Manual</xsl:when>
			<xsl:when test="/pm/@type='spm'">Standard Practices Manual</xsl:when>
			<xsl:when test="/pm/@type='acmm'">Abbreviated Component Maintenance Manual</xsl:when>
			<xsl:when test="/pm/@type='ipc'">Illustrated Parts Catalog</xsl:when>
			<xsl:when test="/pm/@type='gem'">Ground Equipment Manual</xsl:when>
			<xsl:when test="/pm/@type='mm'">Maintenance Manual</xsl:when>
			<xsl:when test="/pm/@type='ohm'">Overhaul Manual</xsl:when>
			<xsl:when test="/pm/@type='lnmm'">Line Maintenance Manual</xsl:when>
			<!-- Default to CMM when no type (or unknown type) specified... shouldn't happen in normal processing. -->
			<!-- UPDATE: Better to signal an error -->
