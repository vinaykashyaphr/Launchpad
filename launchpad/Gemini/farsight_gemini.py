...
def log_print_partial(logText, message, printToLogOnly=False, cc=None):
    if cc is not None:
        cc.print(message, printToLogOnly)
    else:
...
def main(directory, user, logname, **inputs):
    ...
    try:
        cc = ConversionClient(directory, user, logname)
    except Exception:
        cc = None
    ...
    exit_handler = partial(exit_handler_partial, logText, filename=logname, cc=cc)
...
if __name__ == '__main__':
    main(Path('.'), None, 'farsight_gemini.log', d_results=True, d_words=False, d_steps=True, o_files=False, d_process=False, blind=False)
   # input()
