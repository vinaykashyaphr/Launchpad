...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:rx="http://www.renderx.com/XSL/Extensions"
  ...
  <xsl:template name="calc-figure-number-param">
    ...
    <xsl:variable name="figure-position">
      <xsl:choose>
        ...
        <!-- Introduction is not 5-level -->
        <xsl:when test="$figure/ancestor::pmEntry[@pmEntryType='pmt58']">
          <xsl:value-of select="1 + count($figure/ancestor::pmEntry[last()]//figure intersect $figure/preceding::figure)"/>
        </xsl:when>
        <xsl:when test="$documentType='eipc' and $figure/ancestor::pmEntry">
          <!-- For EIPC, restart figure numbering at the 2nd level pmEntry. -->
          <xsl:value-of select="1 + count($figure/ancestor::pmEntry//figure intersect $figure/preceding::figure)"/>
        </xsl:when>
        <xsl:when test="ancestor::pmEntry and $isNewPmc">
          <!-- For 5-level EM, restart figure numbering at the 4th-level pmEntry. -->
          <xsl:value-of select="1 + count($figure/ancestor::pmEntry[count(ancestor::pmEntry)=3]//figure intersect $figure/preceding::figure)"/>
        </xsl:when>
        <xsl:when test="ancestor::pmEntry and not($isNewPmc)">
          <!-- For 3-level EM, restart figure numbering at the 2nd-level pmEntry. -->
          <!-- <xsl:message>Calculating 3-level figure number (title: <xsl:value-of select="$figure/title"/>). Total # figures in the figure's 2nd-level pmEntry: <xsl:value-of select="count($figure/ancestor::pmEntry[count(ancestor::pmEntry)=1]//figure)"/></xsl:message> -->
          <xsl:value-of select="1 + count($figure/ancestor::pmEntry[count(ancestor::pmEntry)=1]//figure intersect $figure/preceding::figure)"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="suffix">
      <xsl:choose>
        <!-- RS: Add section enumeration -->
        <xsl:when test="ancestor::pmEntry/@confnbr">
        	<xsl:text>-</xsl:text>
        	<xsl:value-of select="ancestor::pmEntry/@confnbr"/>
        </xsl:when>
        <!-- Not sure if this is needed for EM... (TODO?) -->
        <!-- Pretty sure it's not needed for 3-level anyway. Comment out for now.
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
      	<!-- <xsl:message>Constructing figure number; prefix: <xsl:value-of select="$prefix"/>; base: <xsl:value-of select="$figure-number-base"/>;
      	  position: <xsl:value-of select="$figure-position"/>; suffix: <xsl:value-of select="$suffix"/></xsl:message> -->
  ...
  <xsl:template name="calc-table-number">
    ...
    <xsl:variable name="table-number-base">
      <xsl:choose>
        <!-- Deal with missing or empty startat values (was causing an XSL error) -->
        <xsl:when test="not(ancestor::pmEntry/@startat) or ancestor::pmEntry[@startat][1]/@startat=''">
          <xsl:value-of select="'0'"/>
        </xsl:when>
        <xsl:when test="ancestor::pmEntry[@startat][1]/@startat &gt; 1">
          <xsl:value-of select="number(ancestor::pmEntry[@startat][1]/@startat) - 1"/>
        </xsl:when>
        <xsl:otherwise>0</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="table-position">
      <xsl:choose>
        <xsl:when test="$documentType='eipc' and ancestor::pmEntry">
          <!-- For EIPC, restart numbering at the 2nd level pmEntry. -->
          <xsl:value-of select="1 + count(ancestor::pmEntry[last()]//table[title] intersect preceding::table[title])"/>
        </xsl:when>
        <!-- Introduction (and earlier) is not 5-level -->
        <xsl:when test="ancestor::pmEntry[@pmEntryType='pmt58'] or ancestor::pmEntry[last()]/following-sibling::pmEntry[@pmEntryType='pmt58']">
          <xsl:value-of select="1 + count(ancestor::pmEntry[last()]//table[title] intersect preceding::table[title])"/>
        </xsl:when>
        <xsl:when test="ancestor::pmEntry and $isNewPmc">
          <!-- For 5-level EM, restart figure numbering at the 4th-level pmEntry. -->
          <xsl:value-of select="1 + count(ancestor::pmEntry[count(ancestor::pmEntry)=3]//table[title] intersect preceding::table[title])"/>
        </xsl:when>
        <xsl:when test="ancestor::pmEntry and not($isNewPmc)">
          <!-- For 3-level EM, restart figure numbering at the 2nd-level pmEntry. -->
          <xsl:value-of select="1 + count(ancestor::pmEntry[count(ancestor::pmEntry)=1]//table[title] intersect preceding::table[title])"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <!-- Put together the table number (with special exception for Transmittal info tables) -->
    <xsl:choose>
      <xsl:when test="ancestor::pmEntry[@pmEntryType='pmt52']">
        <xsl:value-of select="concat('TI-', number($table-number-base + $table-position))"/>
      </xsl:when>
      <xsl:otherwise>
      	<!-- <xsl:message>Constructing table number; prefix: <xsl:value-of select="$table-prefix"/>; base: <xsl:value-of select="$table-number-base"/>;
      	  position: <xsl:value-of select="$table-position"/>; suffix: <xsl:value-of select="$table-suffix"/></xsl:message> -->
