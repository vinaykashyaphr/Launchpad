...
def text_process(text):
    ...
    text = re.sub(r'\((TASK|Subtask)\s*(\w+)\s*-\s*(\w+)\s*-\s*(\w+)\s*-\s*(\w+)\s*-\s*(\w+)\s*-\s*(\w+)\)', r'(\1 \2-\3-\4-\5-\6-\7)', text)
    return text
