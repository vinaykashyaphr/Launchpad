...
<!-- <xsl:template match="PRCLIST1|PRCLIST2|PRCLIST3|PRCLIST4|PRCLIST5|PRCLIST6|PRCLIST7"> -->
 <xsl:template match="mainProcedure">
    <xsl:call-template name="check-rev-start"/>
    <!-- From Styler:
    For EM, add subtask number from the dmRef /@authorityDocument if supplied before the first proceduralStep.
	UPDATE: But not for the new PMC style, since it outputs the subtask after the title
    -->
    <xsl:if test="not($isNewPmc) and $documentType='em' and ancestor::dmContent/preceding-sibling::dmRef[1]/@authorityDocument">
		<fo:block space-after="10pt" space-before="10pt">
			<xsl:text>SUBTASK </xsl:text><xsl:value-of select="ancestor::dmContent/preceding-sibling::dmRef[1]/@authorityDocument"/>
		</fo:block>
    </xsl:if>
    <xsl:apply-templates/>
...
<!--  <xsl:template match="PRCITEM1|PRCITEM2|PRCITEM3|PRCITEM4|PRCITEM5|PRCITEM6|PRCITEM7">
  ...
  <xsl:template match="proceduralStep">
    ...
    <xsl:variable name="childList">
      ...
      <xsl:choose>
	    <!--
        <xsl:when test="following-sibling::PRCLIST2 or following-sibling::PRCLIST3 or following-sibling::PRCLIST4 or following-sibling::PRCLIST5 or following-sibling::PRCLIST6 or following-sibling::PRCLIST7">
    ...
    <fo:block>
      ...
      <xsl:if test="preceding-sibling::node()[not(self::text())][1]/self::processing-instruction('pagebreak')">
      	<xsl:message>Found pagebreak PI before proceduralStep</xsl:message>
		...
		<xsl:if test="not(@changeMark='0') and (@changeType='add' or @changeType='modify')
		  or para[count(preceding-sibling::*)=0][not(@changeMark='0') and (@changeType='add' or @changeType='modify')]">
			<xsl:call-template name="cbStart" />
		</xsl:if>
		
		<!-- Make sure effectivity is reset to "ALL" (later we might want to enable effectivity on proceduralSteps as well... see figure.xsl for how to add it). -->
		<!-- [NOTE: In that case, adding it to all proceduralSteps like this would be problematic, since you might -->
		<!-- have one with a real effectivity on a page, and a whole bunch with "ALL". So it would be better to -->
		<!-- reset the marker after the figure itself. The implementation for proceduralSteps might become complicated.] -->
		<!-- Exception: if the proceduralStep contains a table with effectivity set, don't use "ALL" -->
		<xsl:if test="not(ancestor-or-self::proceduralStep/descendant::table[@applicRefId])">
	  ...
	  <!-- Don't include nested proceduralSteps (ATA has a different nesting structure, with PRCITEM separate (not nesting child lists)) -->
	  <xsl:for-each select="child::*"><!-- [not(self::proceduralStep)] -->
		...
		<xsl:if test="not(@changeMark='0') and (@changeType='add' or @changeType='modify')">
			<!-- UPDATE: Additional test: if a figure is the last item in the proceduralStep, we want to end the change -->
			<!-- bar at the figure, since it adds a page after, which would put the change bar end incorrectly on the page -->
			<!-- after the figure. -->
			<!-- UPDATE: But if the figure has a legend, that would be on the following page, so the change bar -->
			<!-- should continue to include that, so exclude figures with legends from this rule. -->
			<xsl:choose>
				<xsl:when test="(descendant::*)[last()]/ancestor-or-self::figure[not(legend)]">
					<xsl:message>Detected figure ending a changed proceduralStep; not outputting end change bar at proceduralStep level</xsl:message>
				</xsl:when>
				<xsl:otherwise>
					<xsl:message>Did not detect figure ending a changed proceduralStep (last element is <xsl:value-of select="name((descendant::*)[last()])"/>); outputting change bar at proceduralStep level</xsl:message>
	...
	<!-- This might make things out of order if there are any para's etc. occurring after the proceduralStep -->
	<!-- So handling of nested proceduralSteps might need to be moved to template "output-prclist-item" -->
  ...
  <xsl:template name="output-prclist-item">
    ...
    <xsl:choose>
    	<!-- Handle nested proceduralSteps and figures separately -->
    	<xsl:when test="name()='proceduralStep' or name()='figure'">
    	...
    	<xsl:otherwise>
		    ...
		    <fo:list-block xsl:use-attribute-sets="list.vertical.space"
			  ...
			  <!--
		      <xsl:if test="name() = 'TABLE' and ancestor::SUBTASK">
		        <xsl:attribute name="margin-left">
		      ...
		      <fo:list-item>
		        <fo:list-item-label end-indent="label-end()">
		          <fo:block>
					...
					<!--
		            <xsl:if test="preceding-sibling::REGULATORY/REGULATION/@REGTYPE != ''">
		              <xsl:apply-templates select="preceding-sibling::REGULATORY" />
		            </xsl:if>-->
		          </fo:block>
	...
	<xsl:template match="proceduralStep/title">
		<fo:block font-weight="bold" font-size="10pt" 
			...
			<!-- Styler:  Only add the Subtask after the title for the new PMC (the old one adds the subtask before) -->
			<xsl:if test="$isNewPmc and count(ancestor::proceduralStep)=1">
  ...
  <!-- Special case for front-matter levelledPara (not in a level 2 (or greater) pmEntry - from Styler context) -->
  <!-- Just process as normal, and let the nested paras handle any details. -->
  <!-- 2020-10-28 Update Start -->
  <xsl:template match="levelledPara[count(ancestor::pmEntry) = 1]"><!-- not(count(ancestor::pmEntry) = 2) -->
...
<!--    <xsl:if test="false()">
    ...
    <fo:block>
      ...
      <!-- This was causing too much stickiness and gaps in text.
      <xsl:if test="position() = 1 or count(following-sibling::*) = 0">
      ...
      <xsl:if test="position() = 1 and count(descendant::levelledPara) = 0">
        <xsl:attribute name="keep-with-previous.within-column">always</xsl:attribute>
      </xsl:if>
      <!-- If it's the last levelledPara, and there are no nested levelledParas, keep with previous so that -->
      <!-- the last item doesn't appear on its own. -->
      <xsl:if test="count(following-sibling::*) = 0 and count(descendant::levelledPara) = 0">
        <xsl:attribute name="keep-with-previous.within-column">always</xsl:attribute>
      </xsl:if>
      <!-- If it has nested levelledParas, keep with them. -->
      <xsl:if test="count(descendant::levelledPara) &gt; 0">
      ...
      <xsl:for-each select="*">
        <xsl:choose>
          ...
          <xsl:when test="not(name() ='levelledPara')">
	...
	<xsl:template match="attentionRandomList">
	    ...
	    <fo:list-block xsl:use-attribute-sets="list.vertical.space" provisional-distance-between-starts="15pt" space-before="4pt">
	        ...
	        <xsl:choose>
	        	...
	        	<!-- of the notePara. -->
	        	<!-- UPDATE: It looks better with no indent. -->
	        	<xsl:when test="parent::notePara or parent::warningAndCautionPara">
			        <xsl:attribute name="margin-left">0in</xsl:attribute><!-- -0.5in -->
	        	</xsl:when>
	        	<xsl:otherwise>
  ...
  <xsl:template name="calc-proclist-indent">
    ...
    <!-- UPDATE: For 3-level EM, if proceduralSteps occur at the 2nd pmEntry level, also start at 0 -->
    <xsl:choose>
    	<xsl:when test="count(ancestor::pmEntry) = 1 and ancestor::pmEntry/preceding-sibling::pmEntry/@pmEntryType='pmt58'
    		or ( not($documentType='eipc') and count(ancestor::pmEntry) = 2 )">
		    <xsl:choose>
  ...
  <xsl:template name="calc-prclist-position">
    ...
    <xsl:choose>
      ...
      + count(ancestor::dmContent/preceding-sibling::dmContent/dmodule/content/description/levelledPara) + 1"/>
    	...
    	<xsl:otherwise>
			<xsl:value-of select="count(preceding-sibling::proceduralStep) + count(ancestor::dmContent/preceding-sibling::dmContent/dmodule/content/description/levelledPara)
 + 1"/>
    	</xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="get-proclist-format-string">
    ...
    <xsl:choose>
    	...
    	<!-- UPDATE: For 3-level EM, if proceduralSteps occur at the 2nd pmEntry level, also start with "1." -->
    	<xsl:when test="(count(ancestor::pmEntry) = 1 and ancestor::pmEntry/preceding-sibling::pmEntry/@pmEntryType='pmt58')
    		or ( not($documentType='eipc') and count(ancestor::pmEntry) = 2 )">
		    <xsl:choose>
  ...
  <xsl:template name="calc-list-position">
    ...
    <xsl:choose>
      <xsl:when test="$level = 1">
	    <!-- From Styler -->
        <xsl:value-of select="count(preceding-sibling::levelledPara)
  ...
  <!-- Called with LxITEM  or PRCITEM in context -->
