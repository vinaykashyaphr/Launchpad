		  ...
		  <xsl:choose>
			...
			<!-- 2020-08-19 Update End   -->
		  	<xsl:otherwise>
				...
				<xsl:comment>Change start for para</xsl:comment>
		  	</xsl:otherwise>
		  </xsl:choose>
		</xsl:if>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<fo:block> <!--  keep-together.within-page="4"> -->
					<xsl:if test="@id"><xsl:attribute name="id" select="@id"/></xsl:if>
					<!-- Keep together in usual cases... may need to refine condition(s) later. -->
					<!-- Don't apply if in levelledPara: let the levelledPara apply it (the levelledPara number and the text in the para -->
					<!-- were breaking across pages. -->
					...
					<!-- Paras directly in description (but not in front-matter) get an indent (from Styler context) -->
					<xsl:if test="parent::description
					   and not(ancestor::pmEntry[@pmEntryType='pmt52' or @pmEntryType='pmt77' or @pmEntryType='pmt53' or @pmEntryType='pmt54' or @pmEntryType='pmt55'])">
						<xsl:attribute name="start-indent">0.5in</xsl:attribute><!-- 20mm -->
					</xsl:if>
		...
		<xsl:if test="not(@changeMark='0') and (@changeType='add' or @changeType='modify' or @changeType='delete')">
			<xsl:choose>
				...
				<xsl:when test="parent::levelledPara and count(preceding-sibling::*)=0">
			  	...
			  	<xsl:otherwise>
					<xsl:call-template name="cbEnd" />
					<xsl:comment>Change end for para</xsl:comment>
			  	</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
