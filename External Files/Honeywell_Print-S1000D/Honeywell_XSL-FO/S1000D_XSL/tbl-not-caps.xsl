...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Context element should be <table> (within <foldout> parent) -->
  <xsl:template name="foldout-table">

    <xsl:variable name="page-prefix">
      <xsl:call-template name="table-page-prefix"/>
    </xsl:variable>
    
    <xsl:message>FOLDOUT TABLE IN <xsl:value-of select="name(parent::*)"/> (in <xsl:value-of select="name(parent::*/parent::*)"/>)</xsl:message>
    
	<!-- RS: If we're in a sequence of foldout tables, we don't need to close the original page sequences unless we're the first one. -->
	<!-- For now this only works in levelledPara contexts, so apply always if in proceduralSteps. -->
	<!-- UPDATE: Add description context. -->
	<xsl:if test="ancestor::proceduralStep or parent::foldout/parent::description
	  or not(parent::foldout/preceding-sibling::*[1][self::foldout]/table)">
	    <!-- Close original page sequence -->
	    <xsl:call-template name="closeCurrentContext"/>
	</xsl:if>
    
    <!--<xsl:processing-instruction name="ITG">START OF FOLDOUT TABLE PAGE-SEQUENCE</xsl:processing-instruction>-->
    
    <!-- Open new page sequence with apply-templates output inside -->
    <fo:page-sequence master-reference="Foldout" font-family="Arial" font-size="10pt"
      force-page-count="end-on-even" initial-page-number="auto-odd">
      ...
      <fo:flow flow-name="xsl-region-body">
        <fo:block margin-left="-.875in"><!--  break-before="odd-page" -->
	...
	<!-- RS: If we're in a sequence of foldout tables, we don't need to reopen the original page sequence until after the last foldout -->
	<!-- For now this only works in levelledPara contexts, so apply always if in proceduralSteps. -->
	<!-- UPDATE: Add description context. -->
	<xsl:if test="ancestor::proceduralStep or parent::foldout/parent::description
	  or not(parent::foldout/following-sibling::*[1][self::foldout]/table)">
	    <xsl:choose>
	      <xsl:when test="ancestor::proceduralStep or ancestor::levelledPara or parent::foldout/parent::description">
	...
	<!--        <xsl:message>
	      ...
	      For IPL, PGBLK, or Foldout Tables, force-page-count="end-on-even" is needed -->
		    ...
		    <xsl:call-template name="reopenCurrentContext"/>
  ...
  <!-- Close the page sequence for the current context so a foldout or landscape page sequence can -->
  <!-- be inserted. -->
  <xsl:template name="closeCurrentContext">
  
	 <xsl:choose>
	    <xsl:when test="parent::foldout/parent::description">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	            	<!-- Just three blocks for the description context. -->
	    ...
	    <xsl:when test="count(ancestor::proceduralStep)=6">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block></fo:block></fo:block></fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 6) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    <xsl:when test="count(ancestor::proceduralStep)=5">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block></fo:block></fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 5) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    <xsl:when test="count(ancestor::proceduralStep)=4">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block></fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 4) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    <xsl:when test="count(ancestor::proceduralStep)=3">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 3) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    <xsl:when test="count(ancestor::proceduralStep)=2">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 2) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    <xsl:when test="count(ancestor::proceduralStep)=1">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                ...
	                </fo:block></fo:block></fo:block><!-- added for the extra proceduralStep levels (level 1) -->
	              </fo:flow>
	            </fo:page-sequence>]]>              
	          </xsl:text>
	    </xsl:when>
	    
	    <!-- Unlike proceduralSteps, all levelledParas use the same block nesting level in FO. -->
	    
	    <!-- Now foldout tables are no longer wrapped in a list item, so the context is a lot smaller. -->
	    <!-- This might need to be done for landscape tables as well, but they are maintained the same -->
	    <!-- way as before for now (in the following condition). -->
	    <xsl:when test="ancestor::levelledPara and ancestor::foldout">
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                </fo:block></fo:block></fo:block>
	              </fo:flow>
      	...
      	<xsl:when test="ancestor::levelledPara"><!--  and ancestor-or-self::table[@orient='land'] -->
	      <xsl:text disable-output-escaping="yes">
	            <![CDATA[
	                        </fo:block>
	                      </fo:list-item-body>
  ...
  <!-- Restore the context nesting in the original page sequence -->
  <xsl:template name="reopenCurrentContext">
      ...
      <xsl:when test="parent::description">
          ...
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
                  <fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><!-- Rebuild context (level 6) -->
                  <fo:list-block>
                    <fo:list-item>
          ...
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
                  <fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><!-- Rebuild context (level 5) -->
                  <fo:list-block>
                    <fo:list-item>
                      <fo:list-item-label end-indent="label-end()">
                        <fo:block/>
          ...
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
                  <fo:block><fo:block><fo:block><fo:block><fo:block><fo:block><!-- Rebuild context (level 4) -->
                  <fo:list-block>
                    <fo:list-item>
                      <fo:list-item-label end-indent="label-end()">
                        <fo:block/>
          ...
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
                  <fo:block><fo:block><fo:block><fo:block><fo:block><!-- Rebuild context (level 3) -->
                  <fo:list-block>
                    <fo:list-item>
                      <fo:list-item-label end-indent="label-end()">
                        <fo:block/>
      	...
      	<xsl:when test="count(ancestor::proceduralStep)=2"><!-- Rebuild context (level 2) -->
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
      	...
      	<xsl:when test="count(ancestor::proceduralStep)=1"><!-- Rebuild context (level 1) -->
          <xsl:text disable-output-escaping="yes">
              <![CDATA[
                <fo:block>
                  <fo:block><fo:block><fo:block>
        ...
        <fo:table-body>
          <fo:table-row>
            <fo:table-cell padding-before="2in" padding-after="2in" id="ITG_TABLE_FOLDOUT">
              <fo:block>
                <fo:inline>1st FOLDOUT TABLE REPLACEMENT PAGE PLACEHOLDER <xsl:value-of
        ...
        <fo:table-body>
          <fo:table-row>
            <fo:table-cell padding-before="2in" padding-after="2in" id="ITG_NO_COPY">
              <fo:block>
                <fo:inline>2nd FOLDOUT TABLE REPLACEMENT PAGE PLACEHOLDER <xsl:value-of
  ...
  <!-- Calculate the page prefix for foldout and landscape tables (which use their own pageSequence). -->
  <xsl:template name="table-page-prefix">
     <!-- The extra options here are from the original ATA for reference. -->
     <xsl:choose>
  ...
  <!-- Calculate the page suffix for foldout and landscape tables (which use their own pageSequence). -->
  <xsl:template name="table-page-suffix">
     <xsl:choose>
		<xsl:when test="ancestor-or-self::pmEntry[last()]/@authorityName">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@orient='land'">
        ...
        <xsl:variable name="page-suffix">
          <xsl:call-template name="table-page-suffix"/>
        </xsl:variable>
        ...
        <xsl:choose>
          ...
          <xsl:when test="ancestor::proceduralStep or ancestor::levelledPara or parent::description">
            <!--<xsl:message>
              <xsl:text>CJ TEST next page-sequence is because of </xsl:text>
              <xsl:value-of select="following::*[name()='PGBLK' or (name()='TABLE' and (@orient='land' or @tabstyle='hl'))][1]/name()"/>
            </xsl:message>-->
      ...
      <xsl:choose>
        ...
        <xsl:when test="count(preceding-sibling::tgroup) = 0 and not($table-title = 'NO_TITLE')">
          ...
          <fo:table rx:table-omit-initial-header="true" keep-with-previous.within-page="always">
            ...
            <!-- UPDATE: The reason why is that this is how XEP uses a special extension to display continued table headers... see rx:table-omit-initial-header="true" above. -->
  ...
  <xsl:template match="row">
	...
	<!-- Testing pre-process logic:
	<xsl:if test="$parasInFirstEntry &gt; 15">
		<xsl:message>Found entry with more than 15 paras</xsl:message>
		...
		<xsl:message>totalParas: <xsl:value-of select="$totalParas"/>; numEntriesWithEqualParas: <xsl:value-of select="$numEntriesWithEqualParas"/></xsl:message>
		<xsl:if test="$numEntriesWithEqualParas = count(entry)">
			<xsl:message>Found row with all entries have equal number of paras: <xsl:value-of select="$parasInFirstEntry"/></xsl:message>
		</xsl:if>
	</xsl:if> -->
    ...
    <fo:table-row hyphenate="false"><!-- hyphenate="true"  page-break-inside="avoid" -->
      
      <!-- Generally don't allow page breaks in rows, with these exceptions: -->
      <!-- (containing notes with lists, or entries with lots of paras (i.e., tall rows) -->
      <xsl:choose>
      	<xsl:when test=".//note//attentionSequentialList[count(attentionSequentialListItem) &gt; 10]
      		or entry[count(para) &gt; 15]">
          <xsl:attribute name="page-break-inside">auto</xsl:attribute>
      	</xsl:when>
        <xsl:when test=".//sequentialList and (count(following-sibling::row)=0)">
          <xsl:attribute name="page-break-inside">auto</xsl:attribute>
      	</xsl:when>
      	<xsl:when test="entry/@morerows and entry/@morerows &gt; 6">
          <xsl:attribute name="page-break-inside">auto</xsl:attribute>
      	</xsl:when>
      	<xsl:otherwise>
      ...
      <!-- Also keep with next if there is a non-zero morerows attribute. -->
      <!-- UPDATE: If the last row has notes, don't try to keep with it, because some notes can be really long.  -->
      <xsl:if test="( count(following-sibling::row)=1 and not(following-sibling::row[1]//note) )
       or (entry[1]/@morerows and not(entry[1]/@morerows='0'))">
      	<xsl:attribute name="keep-with-next.within-page">1</xsl:attribute><!-- always -->
      </xsl:if>
      <!-- If there is no thead, keep the first row with the next, so the first row doesn't appear by itself -->
      <xsl:if test="not(ancestor::tgroup[1]/thead/row) and count(preceding-sibling::row)=0 and count(following-sibling::row) &gt; 0">
      	<xsl:attribute name="keep-with-next.within-page">1</xsl:attribute>
      </xsl:if>
      
      <!-- Test trying to keep with next for some cases for large rows... didn't work
      <xsl:if test="entry[1]/@morerows">
      	<xsl:message>Forcing row to keep with next due to morerows</xsl:message>
      	<xsl:attribute name="keep-with-next.within-page">always</xsl:attribute>
      </xsl:if> -->
      
      <!-- <xsl:call-template name="ROW"/>--><!-- in "shared": a couple attributes -->
  ...
  <!-- entry-shading: Checks for the added element "cellfont", which Omnimark "fix_pi.xom" -->
  <!-- creates from the Arbortext processing instruction "_cellfont", which results in something like this: -->
  <!--   <cellfont Shading="gray4"/> -->
  <!-- Called with entry in context -->
  <xsl:template name="entry-shading">
  	<xsl:if test="cellfont">
	  	<xsl:variable name="shading" select="cellfont/@Shading"/>
	  	
  		<!-- Based on ATA template "ENTRY-SHADING" in "unhandledElement.xsl" -->
  		<!-- 
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <fo:table-cell xsl:use-attribute-sets="table.cell.padding">
	      <!-- <xsl:call-template name="ENTRY"/> --> <!-- RS: This was in shared/standardVariables.xsl. It over-rides the cell-padding, so removed for now -->
		  <!-- UPDATE: But now ENTRY also invokes the table cell shading template. -->
		  <!-- But the pre-process isn't the same as in ATA, particularly the UPPER XSL is not used, which -->
		  <!-- converts the shading to an attribute. -->
		  <!-- So use a new template here that checks for the added element "cellfont", which Omnimark "fix_pi.xom" -->
		  <!-- creates from the Arbortext processing instruction "_cellfont", which results in something like this: -->
		  <!--   <cellfont Shading="gray4"/> -->
		  <xsl:call-template name="entry-shading"/>
